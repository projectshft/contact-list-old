...
export class AddContact extends Component {
    ...
    onSubmit = (e) => {
        ...
        if (this.state.fname.length > 20) {
            this.setState({errors: { fname: 'First name must be shorter than 20 characters'}});
            return;
        }    
        if (this.state.lname === '') {
        ...
        if (this.state.lname.length > 20) {
            this.setState({errors: { lname: 'Last name must be shorter than 20 characters'}});
            return;
        }
        if (this.state.phone_number === '' || this.state.phone_number.length > 10) {
            this.setState({errors: { phone_number: 'A valid phone number without dashes is required'}});
            return;
        }    
        if (this.state.email === '') {
        ...
        if (this.state.email.indexOf(".") === -1) {
            this.setState({errors: { email: 'Email should contain at least one dot'}});
            return;
        }
    ...
    render() {
        ...
        return (
            <div><h3 className="text-center" style={titleStyle}>Add New Contact</h3>
                <div className="col-md-6 offset-md-3 shadow-sm"style={formStyle}>
                    <div className="row">
                        <form name="addContact" onSubmit={this.onSubmit} onKeyPress={this.handleKeyPress} className="offset-md-4">
                            <TextInputGroup
                                label="First Name"
                                name="fname"
